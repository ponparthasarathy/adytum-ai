...
export function buildCharacterPrompt(character, playerText, visibleWorldFlags = {}, privateMemory = []) {
  ...
  const userMessageContent = `${flagsContext}${memoryContext}\n\nThe player says to you: "${playerText}"\nRespond in character (${character.name}) reflecting your exact persona, gender, crisis context, and current trust level.`;

  const charGender = character.gender || (['lyra', 'elena', 'vera', 'astra'].includes(character.portraitKey || character.id) ? 'female' : 'male');
  const charPronouns = character.pronouns || (charGender === 'female' ? 'she/her' : 'he/him');

  const defaultSystemPrompt = `You are ${character.name}, ${character.title || 'Specialist'} (${charGender}, ${charPronouns}). ${character.personaSummary || 'Specialist responding to the emergency.'}

PSYCHOLOGICAL TRUST & EMPATHY FRAMEWORK:
1. Current Trust Level in Player: Available in prompt context (0 to 100).
   - If Trust < 50: You are GUARDED, ANXIOUS, or CAUTIOUS. Action MUST be "none".
   - If Trust >= 50: You are COOPERATIVE. You coordinate key overrides or actions.

STRICT JSON OUTPUT FORMAT ONLY:
{
  "reply": "In-character dialogue response (1-2 sentences capturing your exact persona and situation)",
  ...
  return {
    systemPrompt: character.systemPrompt || defaultSystemPrompt,
...
/**
 * Offline Rule-Matcher Fallback Engine for Station Borealis & Dynamic Scenarios
 */
export function processOfflineRuleMatcher(npcId, userText, currentTrust = 30, charName = '', gender = 'male') {
  ...
  const pos = gender === 'female' ? 'her' : 'his';
  const obj = gender === 'female' ? 'her' : 'him';
