 ...
 */
export function processOfflineRuleMatcher(npcId, userText, currentTrust = 30) {
  ...
  if (npcId === 'aris') {
    ...
    if (isHostile) {
      reply = 'Dr. Corwin Hale winces in panic: "Shouting won\'t help us escape! Keep your voice down."';
      emotionalState = 'panicked';
    } else if (isEmpathetic || currentTrust >= 50) {
      action = 'override_bypass';
      reply = 'Dr. Corwin Hale adjusts his equipment: "Good thinking. If we work together, we can bypass the pressure locks."';
      emotionalState = 'stabilized';
    } else {
      reply = 'Dr. Corwin Hale checks the console: "We need to stay focused and analyze the situation carefully."';
      emotionalState = 'freezing';
    }
  } else if (npcId === 'wren' || npcId === 'lyra_chen') {
    if (isHostile) {
      reply = 'Lyra Chen / Wren shrinks back: "Please... I\'m doing the best I can under these conditions."';
      emotionalState = 'frightened';
    } else if (isEmpathetic || currentTrust >= 50) {
      action = 'check_systems';
      reply = 'Lyra Chen / Wren brightens up: "Thank you for trusting me. I\'m checking the environmental telemetry now."';
      emotionalState = 'hopeful';
    } else {
      reply = 'Lyra Chen / Wren monitors the display: "The system telemetry is unstable, but I\'m tracking the duct routes."';
      emotionalState = 'weak';
    }
  } else if (npcId === 'sable') {
    if (isHostile) {
      reply = 'Sable smirks coldly: "Hostility won\'t get you anywhere. Let\'s keep things businesslike."';
      emotionalState = 'mocking';
    } else if (isEmpathetic || currentTrust >= 50) {
      action = 'strike_deal';
      reply = 'Sable nods with a sly grin: "Now that\'s a proposal I can work with. Let\'s make a deal."';
      emotionalState = 'cooperative';
    } else {
      reply = 'Sable tilts their head: "Interesting thought, but what\'s in it for me?"';
      emotionalState = 'calculating';
    }
  } else if (npcId === 'garrow' || npcId === 'garrow_haines') {
    if (isHostile) {
      reply = 'Commander Garrow Haines growls: "Mind your tongue. I won\'t tolerate insubordination."';
      emotionalState = 'strict';
    } else if (isEmpathetic || currentTrust >= 50) {
      action = 'request_quarantine_override';
      reply = 'Commander Garrow Haines nods grimly: "Understood. I will authorize temporary access to the security corridor."';
      emotionalState = 'authorizing';
    } else {
      reply = 'Commander Garrow Haines stands guard: "Hold your position while I assess the perimeter threat."';
      emotionalState = 'wary';
    }
  } else {
