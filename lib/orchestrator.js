...
export async function processDialogueRequest({
  ...
  if (forceOffline) {
    const offlineResult = processOfflineRuleMatcher(npcId, playerText, currentTrust, character.name, character.gender || 'male');
  ...
  if (!charLlmSuccess || !reply) {
    const fallback = processOfflineRuleMatcher(npcId, playerText, currentTrust, character.name, character.gender || 'male');
